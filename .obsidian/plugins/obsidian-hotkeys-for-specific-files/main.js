...
var FileSuggest = class extends TextInputSuggest {
  getSuggestions(inputStr) {
    ...
    abstractFiles.forEach((file) => {
      if (file instanceof import_obsidian2.TFile && file.path.toLowerCase().contains(lowerCaseInputStr)) {
        files.push(file);
      }
    });
    return files;
...
function getMomentFromFile(file) {
  if (file.split(".").length > 1) {
    return (0, import_obsidian3.moment)().format(file.substring(0, file.lastIndexOf("."))) + file.substring(file.lastIndexOf("."));
  } else {
    return "";
  }
}
var SpecificFilesPlugin = class extends import_obsidian3.Plugin {
  async onload() {
    ...
    this.app.vault.on("rename", (file, oldPath) => {
      ...
      if (oldItemIndex >= 0) {
        ...
        this.setCommands(this);
        if (hotkeys) {
          this.app.hotkeyManager.setHotkeys(
...
var SettingsTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    let index = 0;
    new import_obsidian3.Setting(containerEl).setName("Prefer existing panes").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("Add command to open file in Hover Editor").setDesc(
    ...
    new import_obsidian3.Setting(containerEl).setName("Add new commands (required after changes)").setDesc(
      "To remove already added commands, you have to reload Obsidian (or disable and enable this plugin) "
    ).addButton(
      (cb) => cb.setButtonText("Add").onClick(() => {
        this.plugin.setCommands(this.plugin);
      }).setClass("mod-cta")
    );
    new import_obsidian3.Setting(containerEl).setName("Create new text field").addButton(
      (cb) => cb.setButtonText("Create").onClick(() => {
        this.addTextField(index);
        index++;
      }).setClass("mod-cta")
    );
    new import_obsidian3.Setting(containerEl).setName("Create new text field with moment format").addButton(
      (cb) => cb.setButtonText("Create").onClick(() => {
  ...
  addTextField(index, fileObjet = { useMoment: false, file: "" }) {
    ...
    if (fileObjet.useMoment) {
      ...
      setting.addText((cb) => {
        ...
        cb.setPlaceholder("Directory/file.md").setValue(fileObjet.file).onChange((value) => {
          this.plugin.settings.files[index] = {
            useMoment: false,
            file: value
          };
